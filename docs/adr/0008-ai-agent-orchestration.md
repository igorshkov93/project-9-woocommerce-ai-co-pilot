...
## Addendum: Optional tool-parameter handling and a known limitation (01.09.2026)
All five fixed Telegram commands (`/ordersqty`, `/top5probs`, `/writeemail`,
`/createcoupon`, `/digestoftheday`) were confirmed live end-to-end in this
session, including the coupon callback branch actually creating a coupon
in WooCommerce and the digest actually reaching Slack. Two related bugs
surfaced during that testing, both instances of the same underlying
pattern: an optional `$fromAI` tool parameter that the model legitimately
omits must be handled as *absent*, consistently, by every node downstream
- not just by the node that declares the default.
- **`build-coupon-button`, round 2**: the fix recorded in the "further
  hardening" addendum above turned out to be incomplete. The node also
  required `toolInput.expires_days` to be present, but the model is
  entitled to omit this optional parameter and let `13a-tool-propose-coupon`
  apply its own default (~14 days). When omitted, `build-coupon-button`
  threw `Required -> at expires_days` and the confirmation button never
  reached the user, even though the underlying proposal had succeeded.
  Fixed by deriving `expires_days` from `observation.payload.date_expires`
  (the tool's actual result, where the default is already applied) via
  `DateTime.fromISO(...).diff($today, 'days').days`, rather than reading
  it from `toolInput` (what the model literally passed). `$now`/`$today`
  are used as the `DateTime` source because `require('luxon')` is
  disallowed in this instance's Code node sandbox. Confirmed live via
  `/createcoupon`.
- **Model-guessed dates instead of an empty optional parameter**: Gemini
  was observed passing `date_from`/`date_to` to `send_orders_digest` with
  the correct month/day but the wrong year (2024 instead of 2026), despite
  the `$fromAI` description explicitly instructing "leave empty to default
  to...". Pinned data and description example dates were both checked and
  ruled out as the cause. This is a model behavior, not a code defect, so
  the fix is in the agent's system prompt: it now explicitly forbids the
  agent from computing or guessing a current date for this tool's optional
  date parameters, instructing it to leave them empty unless the user
  themselves names specific dates. Confirmed live via `/digestoftheday`.
- **Known limitation, left unfixed by decision**: `draft_abandoned_cart_email`
  declares its optional `cart_id` parameter as `$fromAI('cart_id', ...,
  'string', '')`, but the sub-workflow's `prepare-request` node normalizes
  it with `typeof input.cart_id === 'number' ? input.cart_id : null`. Since
  the declared type is `'string'`, any `cart_id` the model does supply
  arrives as a string and never satisfies `typeof === 'number'` - it is
  silently treated as absent, and the tool always falls back to
  auto-selecting the highest-value unrecovered cart. Explicit cart
  targeting is therefore currently unreachable in practice. None of the
  five test scenarios exercise this path, so it was deliberately left as a
  documented known limitation rather than fixed in this session.